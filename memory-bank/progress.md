...
- [x] `frontend.3ai-talk.org` SSO redirect URI and gateway routing fix — `identity/keycloak/realm-shopping-cart.json` now includes `https://frontend.3ai-talk.org/callback` (exact path) in the `frontend` client redirect URIs, and `networking/istio/gateway.yaml` plus `networking/istio/frontend-virtualservice.yaml` now route `frontend.3ai-talk.org`; PR #82 merged to main at SHA `7bf03ae` (2026-06-01); Copilot found wildcard security issue; exact callback path now standard for production; Keycloak pod restart pending.
- [x] Keycloak LDAP bind DN recovery after `ldap-admin` experiment — realm import now re-applies the repo source of truth at Keycloak startup so existing realms pick up the canonical realm import in `identity/keycloak/realm-shopping-cart.json`.
