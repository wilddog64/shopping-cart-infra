...
### Fixed
- `identity/keycloak/keycloak-reconcile-hook-job.yaml`: remove the `awk` dependency from the Keycloak realm reconcile hook because `quay.io/keycloak/keycloak:24.0` is based on `ubi9-micro`, which does not provide `awk`.
