...
### Changed
- `identity/keycloak/`: federate the `shopping-cart` realm against the `openldap` Service (`openldap.identity.svc.cluster.local`, backed by the `openldap-0` StatefulSet pod) on base DN `dc=home,dc=org`, instead of the osixia `ldap` deployment, retiring the osixia bind. `realm-shopping-cart.json` LDAP component now points at `ldap://openldap.identity.svc.cluster.local:389` with `usersDn: ou=users,dc=home,dc=org`, `bindDn: cn=ldap-admin,dc=home,dc=org` and `rdnLDAPAttribute: cn`; `keycloak-secrets-externalsecret.yaml` reads the bind credential from `secret/data/ldap/openldap-admin` (`LDAP_ADMIN_PASSWORD`); `kustomization.yaml` config literals updated to match. The reconcile hook's LDAP group mapper is repointed with it (`groups.dn` → `ou=groups,dc=home,dc=org`, `membership.user.ldap.attribute` → `cn`, matching the seeded `member: cn=...,ou=users,dc=home,dc=org` DNs) so group sync and ArgoCD RBAC keep working. Unifies SSO on the k3d-manager-seeded directory so `get-keycloak-password` and cluster-up seeding describe the same LDAP that Keycloak actually federates.
...
### Fixed
- `identity/keycloak/keycloak-reconcile-hook-job.yaml`: remove the `awk` dependency from the Keycloak realm reconcile hook because `quay.io/keycloak/keycloak:24.0` is based on `ubi9-micro`, which does not provide `awk`. The value-returning CSV helpers end with an explicit `return 0`: under `set -euo pipefail` a `while` loop takes its status from the last command in its body, so `level0_rows` — whose body ended in `[[ ... ]] && printf` — returned 1 whenever the final row was not level 0, and `kcadm` returns a flattened listing whose last row routinely is not. That would have aborted the hook at the same point the missing `awk` did.
- `.github/workflows/build-push-deploy.yml`: fail fast with a named, actionable error when `PROMOTER_SSH_KEY` arrives empty. The secret is declared `required: false`, so a caller that forgot to forward it got an empty key file and a cryptic `Load key "~/.ssh/promoter_key": error in libcrypto` inside the promote step — which is `continue-on-error: true`, so the real cause was buried behind a separate fail gate and invisible on pull requests, where the publish job is skipped entirely. `shopping-cart-product-catalog` promoted no image tag for six consecutive pushes to `main` from 2026-08-12 because of exactly this. The guard names the secret, the block to add, and the repository to check.
- `identity/keycloak/keycloak-reconcile-hook-job.yaml`: reconcile the copied browser flow's inherited level-1 `Browser - Conditional OTP` subflow instead of creating a second top-level `otp-conditional-subflow`. The hook now removes that stray top-level execution, restores the required level-0 execution set and forms/username-password requirements on every run, and exits non-zero if it cannot restore the login-safe invariant. A stray top-level `otp-conditional-subflow` set `CONDITIONAL` beside the `ALTERNATIVE` forms subflow made Keycloak ignore the alternatives, so `Username Password Form` never executed and every authorization request failed with `AuthenticationFlowException` — rendered as a generic "Invalid username or password" page before any login form, breaking frontend and ArgoCD SSO. Flow parsing follows the actual `kcadm` contract, verified live: `authentication/flows/<alias>/executions` returns a *flattened* listing of every nested execution, so the level-0 invariant is checked against a `level`-filtered view; `--format csv` emits **no header row**; a copied flow prefixes its nested subflow names (`browser-with-conditional-otp Browser - Conditional OTP`); `authentication/flows` lists only top-level flows, so subflow paths are derived from the display name and percent-encoded (`kcadm` rejects unencoded spaces); an execution is deleted at `authentication/executions/<id>`; and the role condition's config id is read from the execution's `authenticationConfig` field so a re-run updates the existing config instead of failing on a duplicate create.
